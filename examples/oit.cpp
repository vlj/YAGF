...
class Transparent
    : public ShaderHelperSingleton<Transparent>,
      public TextureRead<UniformBufferResource<0>, ImageResource<0>> {
public:
  Transparent() {
...
class FragmentMerge : public ShaderHelperSingleton<FragmentMerge>,
                      public TextureRead<ImageResource<0>> {
public:
  FragmentMerge() {
    Program = ProgramShaderLoading::LoadProgram(
        GL_VERTEX_SHADER, screenquadshader, GL_FRAGMENT_SHADER, fragmerge);
...
void init() {
  ...
  buffer =
      GeometryCreator::createCubeMeshBuffer(irr::core::vector3df(1., 1., 1.));
  ...
  glBufferData(GL_SHADER_STORAGE_BUFFER, 1000000 * sizeof(PerPixelListBucket),
               0, GL_STATIC_DRAW);
...
void draw() {
  ...
  glBufferData(GL_UNIFORM_BUFFER, sizeof(struct OITBuffer), &cbufdata,
               GL_STATIC_DRAW);
  Transparent::getInstance()->SetTextureUnits(
      cbuf, PerPixelLinkedListHeadTexture, GL_READ_WRITE, GL_R32UI);
  glDrawElementsBaseVertex(GL_TRIANGLES, (GLsizei)buffer->getIndexCount(),
  ...
  FragmentMerge::getInstance()->SetTextureUnits(PerPixelLinkedListHeadTexture,
