...
WrapperResource *SpecularCubemap;
...
void init()
{
  ...
  SpecularCubemap = generateSpecularCubemap(cubemap);

  CommandList = GlobalGFXAPI->createCommandList();

  cbuf = GlobalGFXAPI->createConstantsBuffer(sizeof(Matrixes));
  Inputs = GlobalGFXAPI->createCBVSRVUAVDescriptorHeap({ std::make_tuple(cbuf, RESOURCE_VIEW::CONSTANTS_BUFFER, 0), std::make_tuple(SpecularCubemap, RESOURCE_VIEW::SHADER_RESOURCE, 0) });
