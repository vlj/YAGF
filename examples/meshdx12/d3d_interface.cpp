...
extern "C" {
__declspec(dllexport) void *create_vulkan_mesh(HWND window,
                                               HINSTANCE hInstance);
}
...
void *create_vulkan_mesh(HWND window, HINSTANCE) {
  try {
    ...
    CHECK_HRESULT(D3D12CreateDevice(adaptater.Get(), D3D_FEATURE_LEVEL_11_0,
                                    IID_PPV_ARGS(&dev)));
    ...
    CHECK_HRESULT(fact->CreateSwapChainForHwnd(queue, window, &swapChain,
                                               nullptr, nullptr,
                                               (IDXGISwapChain1 **)&chain));

    uint32_t w, h;
    CHECK_HRESULT(chain->GetSourceSize(&w, &h));
    return new MeshSample(std::make_unique<device_t>(dev),
                          std::make_unique<swap_chain_t>(chain),
                          std::make_unique<command_queue_t>(queue), 1024, 1024,
                          irr::video::ECF_R8G8B8A8_UNORM);
  } catch (...) {
    return nullptr;
  }
}
