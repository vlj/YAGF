...
//        fbo[0] = new D3DRTTSet({ Context::getInstance()->getBackBuffer(0) }, { DXGI_FORMAT_R8G8B8A8_UNORM_SRGB }, 1024, 1024, depth_buffer->D3DValue.resource, &depth_buffer->D3DValue.description.TextureView.DSV);
//        fbo[1] = new D3DRTTSet({ Context::getInstance()->getBackBuffer(1) }, { DXGI_FORMAT_R8G8B8A8_UNORM_SRGB }, 1024, 1024, depth_buffer->D3DValue.resource, &depth_buffer->D3DValue.description.TextureView.DSV);
...
public:
    Sample(HWND window)
    {
        CHECK_HRESULT(D3D12CreateDevice(nullptr, D3D_FEATURE_LEVEL_11_0, IID_PPV_ARGS(dev.GetAddressOf())));
