...
namespace {
DirectX::XMVECTOR g_lightEyePt =
    DirectX::XMVectorSet(-421.25043f, 306.7890949f, -343.22232f, 1.0f);
DirectX::XMVECTOR g_defaultEyePt =
    DirectX::XMVectorSet(-190.0f, 70.0f, -250.0f, 1.0f);
DirectX::XMVECTOR g_defaultLookAt = DirectX::XMVectorSet(0, 40, 0, 1.0f);

// (inv)modelmatrix, jointmatrix
const auto blit_descriptor =
    descriptor_set({range_of_descriptors(RESOURCE_VIEW::SHADER_RESOURCE, 0, 1),
...
auto get_blit_pso(device_t &dev, pipeline_layout_t &layout, render_pass_t &rp) {
  graphic_pipeline_state_description pso_desc =
      graphic_pipeline_state_description::get()
          ...
          .set_vertex_attributes(std::vector<pipeline_vertex_attributes>{
              pipeline_vertex_attributes{0, irr::video::ECF_R32G32F, 0,
                                         4 * sizeof(float), 0},
              pipeline_vertex_attributes{1, irr::video::ECF_R32G32B32F, 0,
...
sample::sample(GLFWwindow *window) {
  ...
  blit_layout = dev->create_pipeline_layout(
      std::vector<const descriptor_set_layout *>{blit_layout_set.get()});
  ...
  big_triangle = dev->create_buffer(
      ...
      usage_vertex);
  float fullscreen_tri[]{-1., -3., 0., 2., 3., 1., 2., 0., -1., 1., 0., 0.};

  memcpy(big_triangle->map_buffer(), fullscreen_tri, 4 * 3 * sizeof(float));
  big_triangle->unmap_buffer();

  descriptor_pool = dev->create_descriptor_storage(
      1, {{RESOURCE_VIEW::SHADER_RESOURCE, 1}, {RESOURCE_VIEW::SAMPLER, 1}});
  descriptor = descriptor_pool->allocate_descriptor_set_from_cbv_srv_uav_heap(
      0, {blit_layout_set.get()}, 2);

  std::transform(back_buffer.begin(), back_buffer.end(),
                 std::back_inserter(back_buffer_view), [&](const auto &img) {
                   return dev->create_image_view(
                       *img, std::get<5>(dev_swapchain_queue), 0, 1, 0, 1,
                       irr::video::E_TEXTURE_TYPE::ETT_2D);
                 });

  fbo[0] = dev->create_frame_buffer(
      std::vector<const image_view_t *>{back_buffer_view[0].get()}, width,
      height, blit_render_pass.get());
  fbo[1] = dev->create_frame_buffer(
      std::vector<const image_view_t *>{back_buffer_view[1].get()}, width,
      height, blit_render_pass.get());

  tressfx_helper.pvkDevice = dynamic_cast<vk_device_t &>(*dev).object;
  tressfx_helper.memoryProperties =
      dynamic_cast<vk_device_t &>(*dev).mem_properties;

  DirectX::XMFLOAT4 lightpos{121.386368f, 420.605896f, -426.585876f,
                             1.00000000f};

  const auto &View = glm::transpose(
      glm::perspectiveFovRH(70.f / 180.f * 3.14f, static_cast<float>(width),
                            static_cast<float>(height), 100.f, 600.f) *
      glm::lookAtRH(glm::vec3(-190.0f, 70.0f, -250.0f),
                    glm::vec3(0.f, 40.f, 0.f), glm::vec3(0.f, -1.f, 0.f)));
  ...
  tressfx_helper.hairParams.color =
      DirectX::XMFLOAT3(98.f / 255.f, 14.f / 255.f, 4.f / 255.f);
  ...
  tressfx_helper.hairParams.ambientLightColor =
      DirectX::XMFLOAT4(.15f, .15f, .15f, 1.f);
  ...
  tressfx_helper.hairParams.pointLightColor =
  ...
  depth_texture = dev->create_image(
      irr::video::D32U8, width, height, 1, 1,
      usage_depth_stencil | usage_sampled | usage_transfer_dst, nullptr);
  depth_texture_view = dev->create_image_view(
      *depth_texture, irr::video::D32U8, 0, 1, 0, 1,
      irr::video::E_TEXTURE_TYPE::ETT_2D, irr::video::E_ASPECT::EA_DEPTH);
  color_texture = dev->create_image(
      irr::video::ECF_R8G8B8A8_UNORM_SRGB, width, height, 1, 1,
      usage_render_target | usage_sampled | usage_transfer_dst, nullptr);
  color_texture_view = dev->create_image_view(
      *color_texture, irr::video::ECF_R8G8B8A8_UNORM_SRGB, 0, 1, 0, 1,
      irr::video::E_TEXTURE_TYPE::ETT_2D, irr::video::E_ASPECT::EA_COLOR);
  ...
  std::unique_ptr<command_list_t> upload_command_buffer =
      command_storage->create_command_list();
  ...
  std::unique_ptr<buffer_t> upload_buffer = dev->create_buffer(
      width * height * 128, irr::video::E_MEMORY_POOL::EMP_CPU_WRITEABLE,
  ...
  bool success = tfxproject.Read(L"..\\..\\..\\deps\\TressFX\\amd_tressfx_"
                                 L"viewer\\media\\ponytail\\ponytail.tfxproj");
  if (!success)
    throw;
  ...
  tressfx_helper.simulationParams.numLengthConstraintIterations =
      tfxproject.lengthConstraintIterations;
  tressfx_helper.simulationParams.numLocalShapeMatchingIterations =
      tfxproject.localShapeMatchingIterations;
  ...
  tressfx_helper.modelTransformForHead =
      DirectX::XMMATRIX(glm::value_ptr(Model));

  for (int i = 0; i < numHairSectionsTotal; ++i) {
    tfxFile.clear();
    tfxFile.open(tfxproject.mTFXFile[i].tfxFileName,
    ...
    guideFollowParams.numFollowHairsPerGuideHair =
        tfxproject.mTFXFile[i].numFollowHairsPerGuideHair;
    guideFollowParams.radiusForFollowHair =
        tfxproject.mTFXFile[i].radiusForFollowHair;
    guideFollowParams.tipSeparationFactor =
        tfxproject.mTFXFile[i].tipSeparationFactor;
    ...
    shapeParam.globalShapeMatchingEffectiveRange =
        tfxproject.mTFXSimFile[i].globalShapeMatchingEffectiveRange;
    shapeParam.stiffnessForGlobalShapeMatching =
        tfxproject.mTFXSimFile[i].stiffnessForGlobalShapeMatching;
    shapeParam.stiffnessForLocalShapeMatching =
        tfxproject.mTFXSimFile[i].stiffnessForLocalShapeMatching;
    tressfx_helper.simulationParams
        .perSectionShapeParams[numHairSectionsCounter++] = shapeParam;
  }

  tressfx_helper.simulationParams.numHairSections = numHairSectionsCounter;

  TressFX_CreateProcessedAsset(
      tressfx_helper, nullptr, nullptr, nullptr,
      dynamic_cast<vk_command_list_t &>(*upload_command_buffer).object,
      dynamic_cast<vk_buffer_t &>(*upload_buffer).object,
      dynamic_cast<vk_buffer_t &>(*upload_buffer).memory);

  upload_command_buffer->set_pipeline_barrier(
      *depth_texture, RESOURCE_USAGE::undefined, RESOURCE_USAGE::DEPTH_WRITE, 0,
      irr::video::E_ASPECT::EA_DEPTH_STENCIL);
  upload_command_buffer->set_pipeline_barrier(
      *color_texture, RESOURCE_USAGE::undefined, RESOURCE_USAGE::READ_GENERIC,
      0, irr::video::E_ASPECT::EA_COLOR);

  upload_command_buffer->set_pipeline_barrier(
      *back_buffer[0], RESOURCE_USAGE::undefined, RESOURCE_USAGE::PRESENT, 0,
      irr::video::E_ASPECT::EA_COLOR);
  upload_command_buffer->set_pipeline_barrier(
      *back_buffer[1], RESOURCE_USAGE::undefined, RESOURCE_USAGE::PRESENT, 0,
      irr::video::E_ASPECT::EA_COLOR);
  ...
  draw_command_buffer->set_pipeline_barrier(
      *depth_texture, RESOURCE_USAGE::DEPTH_WRITE, RESOURCE_USAGE::COPY_DEST, 0,
      irr::video::E_ASPECT::EA_DEPTH_STENCIL);
  draw_command_buffer->set_pipeline_barrier(
      *color_texture, RESOURCE_USAGE::READ_GENERIC, RESOURCE_USAGE::COPY_DEST,
      0, irr::video::E_ASPECT::EA_COLOR);
  draw_command_buffer->clear_depth_stencil(*depth_texture, 1.f);
  draw_command_buffer->clear_color(*color_texture,
                                   std::array<float, 4>{.5f, .5f, .5f, 1.f});

  draw_command_buffer->set_pipeline_barrier(
      *depth_texture, RESOURCE_USAGE::COPY_DEST, RESOURCE_USAGE::DEPTH_WRITE, 0,
      irr::video::E_ASPECT::EA_DEPTH_STENCIL);
  draw_command_buffer->set_pipeline_barrier(
      *color_texture, RESOURCE_USAGE::COPY_DEST, RESOURCE_USAGE::RENDER_TARGET,
      0, irr::video::E_ASPECT::EA_COLOR);
  TressFX_Begin(tressfx_helper, 0);
  TressFX_Simulate(
      tressfx_helper,
      dynamic_cast<vk_command_list_t &>(*draw_command_buffer).object, 0.16f, 0);
  TressFX_RenderShadowMap(
      tressfx_helper,
      dynamic_cast<vk_command_list_t &>(*draw_command_buffer).object, 0);
  draw_command_buffer->set_scissor(0, static_cast<float>(width), 0,
                                   static_cast<float>(height));
  draw_command_buffer->set_viewport(0, static_cast<float>(width), 0.,
                                    static_cast<float>(height), 0., 1.);
  TressFX_Render(tressfx_helper,
                 dynamic_cast<vk_command_list_t &>(*draw_command_buffer).object,
                 0);
  TressFX_End(tressfx_helper);
  draw_command_buffer->set_pipeline_barrier(
      *color_texture, RESOURCE_USAGE::RENDER_TARGET,
  ...
  for (int i = 0; i < 2; i++) {
    ...
    blit_command_buffer[i]->begin_renderpass(*blit_render_pass, *fbo[i], {},
                                             width, height);
    ...
    blit_command_buffer[i]->bind_graphic_descriptor(0, *descriptor,
                                                    *blit_layout);
    blit_command_buffer[i]->bind_vertex_buffers(
        0, {{*big_triangle, 0ull, 4u * static_cast<uint32_t>(sizeof(float)),
             4u * 3u * static_cast<uint32_t>(sizeof(float))}});
...
sample::~sample() { TressFX_Release(tressfx_helper); }
...
void sample::draw() {
  ...
  queue->submit_executable_command_list(
      *blit_command_buffer[current_backbuffer], present_semaphore.get());
  queue->wait_for_command_queue_idle();
