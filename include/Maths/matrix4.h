...
namespace core
{
	...
	public:
		...
		//! Simple operator for directly accessing every element of the matrix.
		const T& operator()(size_t row, size_t col) const { return M[row * 4 + col]; }
		...
		//! Print matrix in classical 4 x 4 array form
		void print() const
		{
...
//		void transformBoxEx(core::aabbox3d<f32>& box) const;
		...
		CMatrix4<T>& buildProjectionMatrixOrthoLH(float left, float right, float up, float down, float zNear, float zFar);
...
//		bool equals(const core::CMatrix4<T>& other, const T tolerance = (T)ROUNDING_ERROR_f64) const;
	...
	*/
	...
	inline core::vector3d<T> CMatrix4<T>::getRotationDegrees() const
	{
		...
		const core::vector3d<float> invScale(1.f / scale.X, 1.f / scale.Y, 1.f / scale.Z);

		float Y;// = -asin(core::clamp(mat[2]*invScale.X, -1.0, 1.0));
		float C = cos(Y);
		Y *= RADTODEG64;
	...
	*/
	...
	inline bool CMatrix4<T>::isIdentity() const
	{
		...
		if (M[0] != (T)1  || M[1] != 0.f || M[2] != 0.f || M[3] != 0.f)
			return false;

		if (M[4] != 0.f || M[ 5] != 1.f || M[6] != 0.f || M[7] != 0.f)
			return false;

		if (M[8] != 0.f || M[9] != 0.f || M[10] != 1.f || M[11] != 0.f)
			return false;
/*
...
//		M[14] = (T)(2.0f*zNear*zFar/(zNear-zFar)); // OpenGL version
	...
	// Builds a left-handed orthogonal projection matrix.
	template <class T>
	inline CMatrix4<T>& CMatrix4<T>::buildProjectionMatrixOrthoLH(float widthOfViewVolume, float heightOfViewVolume, float zNear, float zFar)
	{
	...
	// Builds a left-handed orthogonal projection matrix, with specific corners.
	template <class T>
	inline CMatrix4<T>& CMatrix4<T>::buildProjectionMatrixOrthoLH(float left, float right, float up, float down,
			float zNear, float zFar)
	{
		assert(zNear != zFar); //divide by zero
		assert(right != left); //divide by zero
		assert(up != down); //divide by zero
	...
	// Builds a right-handed orthogonal projection matrix.
	template <class T>
	inline CMatrix4<T>& CMatrix4<T>::buildProjectionMatrixOrthoRH(float widthOfViewVolume, float heightOfViewVolume, float zNear, float zFar)
	{
		assert(widthOfViewVolume != 0.f); //divide by zero
		assert(heightOfViewVolume != 0.f); //divide by zero
		assert(zNear != zFar); //divide by zero
	...
	// Builds a right-handed perspective projection matrix.
	template <class T>
	inline CMatrix4<T>& CMatrix4<T>::buildProjectionMatrixPerspectiveRH(float widthOfViewVolume, float heightOfViewVolume, float zNear, float zFar)
	{
		assert(widthOfViewVolume != 0.f); //divide by zero
		assert(heightOfViewVolume != 0.f); //divide by zero
		assert(zNear != zFar); //divide by zero
	...
	// Builds a left-handed perspective projection matrix.
	template <class T>
	inline CMatrix4<T>& CMatrix4<T>::buildProjectionMatrixPerspectiveLH(float widthOfViewVolume, float heightOfViewVolume, float zNear, float zFar)
	{
		assert(widthOfViewVolume != 0.f); //divide by zero
		assert(heightOfViewVolume != 0.f); //divide by zero
		assert(zNear != zFar); //divide by zero
