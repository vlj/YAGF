...
std::unique_ptr<pipeline_state_t>
get_skinned_object_pipeline_state(device_t &dev, pipeline_layout_t &layout,
                                  render_pass_t &rp) {
#ifdef D3D12
  ...
  CHECK_HRESULT(
      D3DReadFileToBlob(L"skinnedobject.cso", vtxshaderblob.GetAddressOf()));
  psodesc.VS.BytecodeLength = vtxshaderblob->GetBufferSize();
  psodesc.VS.pShaderBytecode = vtxshaderblob->GetBufferPointer();

  CHECK_HRESULT(
      D3DReadFileToBlob(L"object_gbuffer.cso", pxshaderblob.GetAddressOf()));
  ...
  std::vector<D3D12_INPUT_ELEMENT_DESC> IAdesc = {
      ...
      {"TEXCOORD", 0, DXGI_FORMAT_R32G32_FLOAT, 2, 0,
       D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0},
      /*        { "TEXCOORD", 1, DXGI_FORMAT_R32_SINT, 1, 0,
      D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 },
      { "TEXCOORD", 2, DXGI_FORMAT_R32_FLOAT, 1, 4,
      D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 },
      { "TEXCOORD", 3, DXGI_FORMAT_R32_SINT, 1, 8,
      D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 },
      { "TEXCOORD", 4, DXGI_FORMAT_R32_FLOAT, 1, 12,
      D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 },
      { "TEXCOORD", 5, DXGI_FORMAT_R32_SINT, 1, 16,
      D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 },
      { "TEXCOORD", 6, DXGI_FORMAT_R32_FLOAT, 1, 20,
      D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 },
      { "TEXCOORD", 7, DXGI_FORMAT_R32_SINT, 1, 24,
      D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 },
      { "TEXCOORD", 8, DXGI_FORMAT_R32_FLOAT, 1, 28,
      D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 }*/
  };
  ...
  psodesc.NodeMask = 1;
  CHECK_HRESULT(dev->object->CreateGraphicsPipelineState(
  ...
  graphic_pipeline_state_description pso_desc =
      graphic_pipeline_state_description::get()
          ...
          .set_vertex_attributes(std::vector<pipeline_vertex_attributes>{
              pipeline_vertex_attributes{0, irr::video::ECF_R32G32B32F, 0,
                                         sizeof(aiVector3D), 0},
              pipeline_vertex_attributes{1, irr::video::ECF_R32G32B32F, 1,
                                         sizeof(aiVector3D), 0},
              pipeline_vertex_attributes{2, irr::video::ECF_R32G32F, 2,
                                         sizeof(aiVector3D), 0}})
          .set_color_outputs(
              std::vector<color_output>{{false}, {false}, {false}});

  return dev.create_graphic_pso(pso_desc, rp, layout, 0);
}

std::unique_ptr<pipeline_state_t>
get_sunlight_pipeline_state(device_t &dev, pipeline_layout_t &layout,
                            render_pass_t &rp) {
  ...
  CHECK_HRESULT(
      D3DReadFileToBlob(L"sunlight.cso", pxshaderblob.GetAddressOf()));
  psodesc.PS.BytecodeLength = pxshaderblob->GetBufferSize();
  psodesc.PS.pShaderBytecode = pxshaderblob->GetBufferPointer();
  ...
  graphic_pipeline_state_description pso_desc =
      graphic_pipeline_state_description::get()
          ...
          .set_vertex_attributes(std::vector<pipeline_vertex_attributes>{
              ...
              {1, irr::video::ECF_R32G32F, 0, 4 * sizeof(float),
               2 * sizeof(float)}})
...
std::unique_ptr<pipeline_state_t>
get_ibl_pipeline_state(device_t &dev, pipeline_layout_t &layout,
                       render_pass_t &rp) {
#ifdef D3D12
  pipeline_state_t result;
  D3D12_GRAPHICS_PIPELINE_STATE_DESC psodesc(get_pipeline_state_desc(pso_desc));
  ...
  graphic_pipeline_state_description pso_desc =
      graphic_pipeline_state_description::get()
          ...
          .set_vertex_attributes(std::vector<pipeline_vertex_attributes>{
              ...
              {1, irr::video::ECF_R32G32F, 0, 4 * sizeof(float),
               2 * sizeof(float)}})
          .set_color_outputs(std::vector<color_output>{
              {true, blend_op::add, blend_factor::one, blend_factor::one,
               blend_factor::one, blend_factor::one}});

  return dev.create_graphic_pso(pso_desc, rp, layout, 0);

  /*	VkPipelineColorBlendAttachmentState blend_attachment_state{ true,
  VK_BLEND_FACTOR_ONE, VK_BLEND_FACTOR_ONE , VK_BLEND_OP_ADD,
  VK_BLEND_FACTOR_ONE , VK_BLEND_FACTOR_ONE , VK_BLEND_OP_ADD, -1 };
  VkPipelineColorBlendStateCreateInfo blend_state{
  VK_STRUCTURE_TYPE_PIPELINE_COLOR_BLEND_STATE_CREATE_INFO, nullptr, 0, false,
  VK_LOGIC_OP_NO_OP, 1, &blend_attachment_state };*/
}

std::unique_ptr<pipeline_state_t>
get_skybox_pipeline_state(device_t &dev, pipeline_layout_t &layout,
                          render_pass_t &rp) {
#ifdef D3D12
  pipeline_state_t result;
  D3D12_GRAPHICS_PIPELINE_STATE_DESC psodesc(get_pipeline_state_desc(pso_desc));
  ...
  Microsoft::WRL::ComPtr<ID3DBlob> vtxshaderblob, pxshaderblob;
  CHECK_HRESULT(
      D3DReadFileToBlob(L"skyboxvert.cso", vtxshaderblob.GetAddressOf()));
  psodesc.VS.BytecodeLength = vtxshaderblob->GetBufferSize();
  psodesc.VS.pShaderBytecode = vtxshaderblob->GetBufferPointer();
