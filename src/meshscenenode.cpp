...
#include "..\include\Scene\MeshSceneNode.h"
	...
	namespace scene
	{
		...
		*/
			...
			: ISceneNode(parent, position, rotation, scale)
		{
			...
			total_index_cnt = 0;
			for (int i = 0; i < importer->mNumMeshes; ++i)
			{
				const aiMesh *mesh = importer->mMeshes[i];
			...
			object_data_buffer = create_buffer(dev, sizeof(ObjectData));
			util::update_descriptor_sets(dev->object,
			{
				...
				{ VkDescriptorBufferInfo{ object_data_buffer->object, 0, sizeof(ObjectData) } }, 0),
				/*		structures::write_descriptor_set(object_descriptor_set, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,
			...
			for (int texture_id = 0; texture_id < importer->mNumMaterials; ++texture_id)
			{
				aiString path;
				importer->mMaterials[texture_id]->GetTexture(aiTextureType_DIFFUSE, 0, &path);
				...
				std::tie(texture, upload_buffer) = load_texture(dev, fixed, initialisation_command);
		...
		void IMeshSceneNode::fill_draw_command_list(command_list_t current_cmd_list, pipeline_layout_t object_sig)
		{
		...
		void IMeshSceneNode::update_object_mesh_data(device_t dev)
		{
			ObjectData *cbufdata = static_cast<ObjectData*>(map_buffer(dev, object_data_buffer));
			...
			unmap_buffer(dev, object_data_buffer);
