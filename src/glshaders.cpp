...
public:
  ObjectShader() {
    ...
    const std::string &vs = std::string((std::istreambuf_iterator<char>(vsin)),
                                        std::istreambuf_iterator<char>());

    std::ifstream fsin("../examples/shaders/object_gbuffer.frag", std::ios::in);
    const std::string &fs = std::string((std::istreambuf_iterator<char>(fsin)),
                                        std::istreambuf_iterator<char>());

    Program = ProgramShaderLoading::LoadProgram(GL_VERTEX_SHADER, vs.c_str(),
...
class Sunlight : public ShaderHelperSingleton<Sunlight>,
                 TextureRead<UniformBufferResource<0>, UniformBufferResource<1>,
                             TextureResource<GL_TEXTURE_2D, 0>,
                             TextureResource<GL_TEXTURE_2D, 1>,
                             TextureResource<GL_TEXTURE_2D, 2>> {
public:
  Sunlight() {
    ...
    AssignSamplerNames(Program, "VIEWDATA", "LIGHTDATA", "ntex", "ctex",
                       "dtex");
  }
};
...
class Tonemap : public ShaderHelperSingleton<Tonemap>,
                TextureRead<TextureResource<GL_TEXTURE_2D, 0>> {
public:
  Tonemap() {
    std::ifstream vsin("../examples/shaders/screenquad.vert", std::ios::in);
    const std::string &vs = std::string((std::istreambuf_iterator<char>(vsin)),
...
class Skybox
    : public ShaderHelperSingleton<Skybox>,
      TextureRead<UniformBufferResource<0>, TextureResource<GL_TEXTURE_2D, 0>> {
public:
  Skybox() {
...
class ImportanceSamplingForSpecularCubemap
    : public ShaderHelperSingleton<class ImportanceSamplingForSpecularCubemap>,
      TextureRead<UniformBufferResource<0>, TextureResource<GL_TEXTURE_2D, 0>,
                  TextureResource<GL_TEXTURE_2D, 1>> {
public:
  ImportanceSamplingForSpecularCubemap() {
    ...
    std::ifstream fsin("../examples/shaders/importance_sampling_specular.frag",
                       std::ios::in);
    const std::string &fs = std::string((std::istreambuf_iterator<char>(fsin)),
                                        std::istreambuf_iterator<char>());
    Program = ProgramShaderLoading::LoadProgram(GL_VERTEX_SHADER, vs.c_str(),
...
struct WrapperPipelineState *ImportanceSamplingForSpecularCubemap() {
  WrapperPipelineState *result =
      (WrapperPipelineState *)malloc(sizeof(WrapperPipelineState));
  result->GLValue.Program =
      ImportanceSamplingForSpecularCubemap::getInstance()->Program;
  result->GLValue.StateSetter = ImportanceSamplingForSpecularCubemapStateSetter;
  return result;
}

class ComputeSHShader
    : public ShaderHelperSingleton<ComputeSHShader>,
      TextureRead<UniformBufferResource<0>, TextureResource<GL_TEXTURE_2D, 0>> {
public:
  ComputeSHShader() {
    ...
    const std::string &fs = std::string((std::istreambuf_iterator<char>(fsin)),
                                        std::istreambuf_iterator<char>());
    Program = ProgramShaderLoading::LoadProgram(GL_COMPUTE_SHADER, fs.c_str());

    AssignSamplerNames(Program, "DATA", "probe");
    GLuint SHblock =
        glGetProgramResourceIndex(Program, GL_SHADER_STORAGE_BLOCK, "SH");
...
class IBLShader
    : public ShaderHelperSingleton<class IBLShader>,
      TextureRead<
          UniformBufferResource<0>, UniformBufferResource<1>,
          TextureResource<GL_TEXTURE_2D, 0>, TextureResource<GL_TEXTURE_2D, 1>,
          TextureResource<GL_TEXTURE_2D, 2>, TextureResource<GL_TEXTURE_2D, 3>,
          TextureResource<GL_TEXTURE_2D, 4>> {
public:
  IBLShader() {
    std::ifstream vsin("../examples/shaders/screenquad.vert", std::ios::in);
    const std::string &vs = std::string((std::istreambuf_iterator<char>(vsin)),
    ...
    AssignSamplerNames(Program, "VIEWDATA", "IBLDATA", "ntex", "ctex", "dtex",
                       "probe", "dfg");
  }
};
